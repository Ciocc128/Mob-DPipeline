...
valid_subjects = data_quality_fl[data_quality_fl > 1]
...
fldata_all = TVSFreeLivingDataset(dataset_path, reference_system="INDIP", missing_reference_error_type="skip")
...
fldata = fldata_all.get_subset(participant_id = valid_IDs)
...
aggregated_paras = {}
ref_parameters = {}
for trial in tqdm(fldata): # tqdm -> adds progress bar
...
per_wb_paras.index.names= ["cohort", "participant_id", "tm", "recording_id", "wb_id"]
per_wb_paras.index.names
ref_parameters.index.names= ["cohort", "participant_id", "tm", "recording_id", "wb_id"]
...
per_trial_participant_day_grouper = create_multi_groupby(
    detected_dmo,
    reference_dmo,
    groupby=["cohort", "participant_id", "tm", "recording_id"],
)
...
# Save and read
wb_matches_with_errors.to_json('wb_matches_with_errors_TPE_FL.json')
wb_matches_with_errors2 = pd.read_json('wb_matches_with_errors_TPE_FL.json')
...
agg_results = agg_results.T

# Combined evaluation
# group detected and reference DMOs of each WB according to Recording

test_matches = (
    combined_dmos.groupby(
        level=["cohort", "participant_id", "tm", "recording_id"], axis=0
    )
    .median()
    .dropna()
)
...
selected_rows = wb_matches_with_errors2["end"]["detected"]
